"""Git operations for ZERG branch management and merging."""
...
class GitOps:
    ...
    def commit(
        ...
        logger.info(f"Created commit {commit[:8]}: {message[:50]}")
        return commit
    ...
    def merge(
        ...
        commit = self.current_commit()
        logger.info(f"Merged {branch} into {self.current_branch()}: {commit[:8]}")
        return commit
