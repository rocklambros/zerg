# DC12-001: Create Test Infrastructure

Create `tests/integration/conftest_container.py` with shared fixtures for container testing.

## Files Owned
- `tests/integration/conftest_container.py`

## Fixtures Required

1. `docker_available()` - Helper function to check Docker availability
2. `mock_docker_run` - Pytest fixture that patches subprocess.run for Docker
3. `multi_lang_project(tmp_path)` - Creates project with multiple language markers
4. `devcontainer_output_dir(tmp_path)` - Creates .devcontainer directory
...
```bash
python -c "from tests.integration.conftest_container import docker_available, multi_lang_project"
```

## Success Criteria
- File exists at `tests/integration/conftest_container.py`
- All fixtures importable without error
- `docker_available()` returns bool without raising
