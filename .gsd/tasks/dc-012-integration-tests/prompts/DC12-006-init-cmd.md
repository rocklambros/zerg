# DC12-006: TestInitCommand

Create `tests/integration/test_container_init_cmd.py` with 1 test for init command devcontainer creation.

## Files Owned
- `tests/integration/test_container_init_cmd.py`

## Dependencies
- DC12-003 (devcontainer generation tests must pass)

## Tests Required

1. `test_init_creates_multi_lang_devcontainer` - Verify init creates proper devcontainer for multi-language project
    ...
    def test_init_creates_multi_lang_devcontainer(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            if devcontainer_path.exists():
                ...
                assert "features" in config or "image" in config, \
                    f"Config missing features/image: {config.keys()}"
        finally:
...
```bash
pytest tests/integration/test_container_init_cmd.py -v
```

## Success Criteria
- Test passes
- Init command completes without error
- devcontainer.json created with valid structure
