# DC12-007: TestRushCommand

Create `tests/integration/test_container_rush_cmd.py` with 1 test for rush command --mode flag.

## Files Owned
- `tests/integration/test_container_rush_cmd.py`

## Dependencies
- DC12-005 (orchestrator mode selection tests must pass)

## Tests Required

1. `test_rush_help_shows_mode_option` - Verify --mode flag appears in rush help output
...
```bash
pytest tests/integration/test_container_rush_cmd.py -v
```

## Success Criteria
- Test passes
- --mode flag visible in help
- All three mode options documented (subprocess, container, auto)
